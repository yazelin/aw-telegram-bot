...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(["git", "config", "user.email",
                       ...
                       cwd=tmpdir, capture_output=True)
        # Detect default branch name (forks may use master instead of main)
        ...
        subprocess.run(["git", "commit", "-m", "Initial commit: project setup"],
                       cwd=tmpdir, capture_output=True)
        result = subprocess.run(["git", "push", "origin", default_branch],
                               cwd=tmpdir, capture_output=True, text=True)
        if result.returncode != 0:
            print(json.dumps({"ok": False, "error": f"Push failed: {result.stderr.strip()[-300:]}"}))
            sys.exit(1)

    # Enable GitHub Pages (deploy from default branch root)
    result = subprocess.run(
        ["gh", "api", f"repos/{repo}/pages", "-X", "POST",
         "-f", "build_type=legacy", "-f", f"source[branch]={default_branch}", "-f", "source[path]=/"],
        capture_output=True, text=True
    )
